...
import Beans.Divisa;
import Persistencia.DataAdapters.tblDivisaAdapter;
import java.net.URL;
...
public class SceneDivisasController implements Initializable {
    ...
    void mniBorrarActionPerformed(ActionEvent event) {
        if (divisaActual != null) {
            if (JOptionPane.showConfirmDialog(null,
                    "¿Desea borrar " + divisaActual.getNombre() + "?",
                    "A T E N C I Ó N",
                    JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
                tbldivisaadapter.Delete(divisaActual);
                reinicializarCampos();
            }
        } else {
    ...
    void btnGuardar_ActionPerformed(ActionEvent event) {
        if (registroNuevo) {
            ...
            divisaActual.setNombre(txtNombre.getText());
            divisaActual.setValor(Double.parseDouble(txtValor.getText()));
            tbldivisaadapter.Update(divisaActual);
        }
    ...
    void initialize() {
        ...
        assert mniBorrar != null : "fx:id=\"mniBorrar\" was not injected: check your FXML file 'sceneDivisas.fxml'.";
